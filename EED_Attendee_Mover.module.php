...
class EED_Attendee_Mover extends EED_Module
{
    ...
    public static function set_hooks_admin()
    {
        add_action(
            ...
            ['EED_Attendee_Mover', 'edit_attendee_selections_init']
        );
        add_filter(
            'FHEE__EE_Registrations_List_Table__column_actions__actions',
            ['EED_Attendee_Mover', 'edit_attendee_selections_button_reg_list'],
            10,
            2
        );
        add_action(
        ...
        add_action(
            ...
            ['EED_Attendee_Mover', 'registration_moved_notice'],
            10,
            1
        );
        add_filter(
            'FHEE__Extend_Registrations_Admin_Page__page_setup__page_routes',
            ['EED_Attendee_Mover', 'attendee_mover_page_routes'],
            10,
            2
        );
        add_filter(
            'FHEE__Extend_Registrations_Admin_Page__page_setup__page_config',
            ['EED_Attendee_Mover', 'attendee_mover_page_config'],
            10,
            2
        );
        add_filter(
            'FHEE__EE_Admin_Page___display_legend__items',
            ['EED_Attendee_Mover', 'reg_admin_list_legend'],
            10,
            1
        );
    }
    ...
    public static function attendee_mover_page_routes(array $page_routes, Registrations_Admin_Page $admin_page)
    {
        ...
        $TKT_ID                                     = ! empty($req_data['TKT_ID']) && ! is_array($req_data['TKT_ID'])
            ? $req_data['TKT_ID']
            : 0;
        $page_routes['edit_attendee_selections']    = [
            'func'       => ['EED_Attendee_Mover', 'edit_attendee_selections'],
        ...
        $page_routes['process_attendee_selections'] = [
            'func'       => ['EED_Attendee_Mover', 'process_attendee_selections'],
            'args'       => [$admin_page],
            'capability' => 'ee_edit_registration',
            'obj_id'     => $REG_ID,
            '_REG_ID'    => $REG_ID,
    ...
    public static function attendee_mover_page_config($page_config, Registrations_Admin_Page $admin_page)
    {
        EED_Attendee_Mover::$admin_page          = $admin_page;
        $req_data                                = $admin_page->get_request_data();
        $page_config['edit_attendee_selections'] = [
            'nav'           => [
            ...
            'metaboxes'     => array_merge(
                ...
                [['EED_Attendee_Mover', 'add_edit_attendee_selections_meta_box']]
            ),
            'require_nonce' => true,
        ];
        return $page_config;
    ...
    public static function add_edit_attendee_selections_meta_box()
    {
        add_meta_box(
            ...
            ['EED_Attendee_Mover', 'edit_attendee_selections_meta_box'],
     ...
     * @throws EE_Error
     */
    public static function edit_attendee_selections_button_reg_list(array $actions, EE_Registration $registration)
    {
        if (
            ! in_array(
    ...
    public static function registration_moved_notice($REG_ID)
    {
        ...
        foreach ($reg_moved_meta as $to_or_from => $reg_meta) {
            $reg_moved = $registration->get_extra_meta($to_or_from, true, []);
     ...
     */
    public static function edit_attendee_selections(EE_Admin_Page $admin_page)
    {
